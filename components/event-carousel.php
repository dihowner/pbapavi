    ...
    <div id="default-carousel" class="order-1 lg:order-2 relative w-full ml-auto mb-10 lg:mb-0" data-carousel="slide">
        ...
        <div class="relative  h-56 overflow-hidden rounded-xl lg:h-96">
            <?php foreach ($eventImages as $image) { ?>
                <div class="hidden duration-700 ease-in-out" data-carousel-item>
                    <img src="<?php echo UPLOADS_DIR . EVENT_DIR .$image; ?>" class="absolute block w-full h-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="...">
                </div>
            <?php } ?>
        </div>
        <!-- Slider indicators -->
        <div class="absolute z-30 flex -translate-x-1/2 bottom-5 left-1/2 space-x-3 rtl:space-x-reverse">
            <?php for ($i = 0; $i < count($eventImages); $i++) { ?>
                <button type="button" class="w-3 h-3 rounded-full" aria-current="<?php echo ($i == 0) ? 'true' : 'false'; ?>" aria-label="Slide <?php echo $i + 1; ?>" data-carousel-slide-to="<?php echo $i; ?>"></button>
            <?php } ?>
        </div>
